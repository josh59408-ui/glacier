...
class GoogleSheetExporter:
    ...
    def _apply_new_old_colors(
        ...
        try:
            ...
            logger.warning(f"新/舊背景色標記失敗（不影響資料匯出）: {e}")
    ...
    def _export_verification_vcp(
        ...
        try:
            ...
            logger.info(f"VCP 驗證資料匯出完成: {len(vcp_data)} 筆 -> {tab_name}")
            return True
    ...
    def sort_worksheets_by_date(
        ...
        try:
            ...
            # 日期頁籤按名稱降序排列（最新在前，同日期 VCP 在三線前面）
